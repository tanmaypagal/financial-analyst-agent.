...
* **Owner decisions.** The PE hard deal-size cap is **USD 10bn enterprise value** (`largest_practical_deal_ev_usd`, set by the owner because several companies in the data are far too large to be bought out; the PE screen is a hypothetical exercise and above-cap names are only ever hypothetical cases). At that cap only Kratos is within reach in Defense, 3 companies in Tech and 7 in Logistics. Every other threshold in the persona YAMLs is a default, not a conclusion.
...
* **Size.** <!-- AUTO:counts -->
**133 automated tests** (`pytest -q`, no API key needed) | **31 eval cases** (`python -m eval.run_eval`) | **11 MCP tools**
<!-- /AUTO:counts -->
...
Persona YAMLs (`config/personas/*.yaml`) hold everything that differs by persona: `system_prompt`, `lens_description`, `priority_metrics` (drive the tool plan), `thresholds`, `required_sections`, `output_schema_extension` (JSON Schema for `persona_output`), `forbidden_reasoning`. **All thresholds are defaults for the owner to review.** In particular the PE persona has a **hard deal-size cap** `largest_practical_deal_ev_usd` = **USD 10bn** (owner's decision): above it `lbo_score` is at most 4, `key_blockers` must contain "exceeds practical sponsor deal size", the company can never be `top_pick`, and it may only be discussed as a hypothetical case. The size comes from the server (`enterprise_value_usd` in `get_valuation` and as a `get_sector_stats` metric), and `eval/checks.py::check_deal_size_cap` verifies the rule on every PE answer.
